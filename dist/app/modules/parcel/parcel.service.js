...
const createParcel = (Payload, decodedUser) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const receiver = yield user_model_1.User.findOne({ email: Payload.receiverEmail }).session(session);
        if (!receiver || receiver.role !== user_interface_1.Role.RECEIVER) {
            throw new AppError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Invalid receiver.');
        }
...
const updateParcel = (parcelId, payload, decodedUser) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const updatedParcel = yield parcel_model_1.Parcel.findByIdAndUpdate(parcelId, senderPayload, { new: true, runValidators: true, });
...
const getAllParcels = (decodedUser, allQuery) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    else if (user.role === user_interface_1.Role.ADMIN) {
        query = {};
    }
    const parcels = parcel_model_1.Parcel.find(query);
    const queryBuilder = new QueryBuilder_1.QueryBuilder(parcels, allQuery);
...
const receiverIncomingParcels = (decodedUser) => __awaiter(void 0, void 0, void 0, function* () {
    const user = yield user_model_1.User.findById(decodedUser.userId);
    if (!user) {
        throw new AppError_1.default(http_status_codes_1.StatusCodes.NOT_FOUND, 'User not found.');
    }
    ...
    const data = {
        ...
        currentStatus: { $ne: parcel_interface_1.ParcelStatus.DELIVERED },
    };
    const incomingParcels = yield parcel_model_1.Parcel.find(data);
    const totalIncomingParcels = yield parcel_model_1.Parcel.countDocuments(data);
    return {
...
const confirmedDelivery = (parcelId, decodedUser) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (parcel.currentStatus === parcel_interface_1.ParcelStatus.CONFIRMED) {
            throw new AppError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, 'Parcel has already been confirmed.');
        }
...
const deliveryHistory = (decodedUser) => __awaiter(void 0, void 0, void 0, function* () {
    const user = yield user_model_1.User.findById(decodedUser.userId);
    if (!user) {
        throw new AppError_1.default(http_status_codes_1.StatusCodes.UNAUTHORIZED, 'User not found.');
    }
    let deliveredParcel;
    if (user.role === user_interface_1.Role.RECEIVER) {
        deliveredParcel = yield parcel_model_1.Parcel.find({
            receiverEmail: user.email,
            currentStatus: { $in: [parcel_interface_1.ParcelStatus.DELIVERED, parcel_interface_1.ParcelStatus.CONFIRMED, parcel_interface_1.ParcelStatus.IN_TRANSIT, parcel_interface_1.ParcelStatus.CANCELLED] },
        });
    }
    else if (user.role === user_interface_1.Role.SENDER) {
        deliveredParcel = yield parcel_model_1.Parcel.find({
            senderId: user._id,
            currentStatus: { $in: [parcel_interface_1.ParcelStatus.DELIVERED, parcel_interface_1.ParcelStatus.CONFIRMED, parcel_interface_1.ParcelStatus.CANCELLED, parcel_interface_1.ParcelStatus.APPROVED, parcel_interface_1.ParcelStatus.IN_TRANSIT] },
        });
    }
    else {
